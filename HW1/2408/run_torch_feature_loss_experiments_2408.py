...
from run_torch_experiments import (
    ORIGINAL_NOTEBOOK,
    build_dataset_cell,
...
BASE_RESULTS = ROOT / "experiments" / "results" / "all_experiments.json"
SWEEP_RESULTS = ROOT / "experiments_2408_lb_sweep" / "results" / "all_experiments.json"
REFINE_RESULTS = ROOT / "experiments_2408_lb_refine" / "results" / "all_experiments.json"
ADVANCED_ROOT = ROOT / "experiments_2408_feature_loss"
...
def build_reference_entries(base_data, sweep_data, refine_data):
    ...
    v9 = {
        ...
        "source": "experiments_2408_lb_sweep/results/all_experiments.json",
    }
    ...
    v14 = {
        ...
        "source": "experiments_2408_lb_refine/results/all_experiments.json",
    }
...
"""
    ...
    nb.cells[24].source = (
        "**Reasoning**:\n"
        f"Fix `look_back={config['look_back']}` at the current 2408 winner, then test `feature_mode={feature_mode}` with `loss_name={loss_name}`."
    )
    ...
    nb.cells[30].source = (
        "**Reasoning**:\n"
        "Generate predictions and compare them with the same evaluation flow used by the existing 2408 runs."
    )
        ...
        ranking.append(
            {
                ...
                "source": entry.get("source", "experiments_2408_feature_loss"),
...
def main():
    ...
    best_new_experiment = min(
        (
            {
                "version": entry["version"],
                "source": "experiments_2408_feature_loss",
                "notebook": entry["notebook"],
                "look_back": entry["config"]["look_back"],
